...
export default {
	...
	props: {
		...
		/**
		 * Maps to Vuetify 'height' prop.
		 *
		 * @alias module:vuetify-google-autocomplete.props.height
		 * @see {@link https://vuetifyjs.com/en/components/text-fields}
		 * @type {Number|String}
		 */
		height: {
	...
	methods: {
		...
		focus() {
			this.$nextTick(() => {
				// this.$refs.autocomplete.focus();  //無法使用
		...
		blur() {
			this.$nextTick(() => {
				// this.$refs.autocomplete.blur();  //無法使用
				// document.getElementsByName(this.id)[0].blur();
